        ...
        while (not q.empty()):
            ...
            for name in this_dot.succ:
                ...
                if dot.initial_idle == None:
                    ...
                    if dot.schedule_class not in self.schedule_class_dict:
                        self.schedule_class_dict[dot.schedule_class] = dot.initial_idle - dot.schedule_idx
                    else:
                        dot.adjust_initial_idle(self.schedule_class_dict)
                    q.put(dot.name)

            for name in this_dot.pre:
                dot: OperateDot = self.dot_dict[name]
                if dot.initial_idle == None:
                    dot.calculate_initial_idle_by_succ(this_dot.initial_idle, self,schedule_class_dict):
                    q.put(dot.name)
